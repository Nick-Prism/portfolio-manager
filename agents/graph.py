...
from agents.arbitrage import run_arbitrage_agent
...
class AgentState(TypedDict):
    ...
    ohlcv: Any
    returns: Any
    benchmark_returns: Optional[Any]
    ...
    arbitrage: Optional[ArbitrageSignal]
...
def arbitrage_node(state: AgentState) -> dict:
    try:
        result = run_arbitrage_agent(symbol=state["symbol"])
        return {"arbitrage": result}  # None is fine — optional signal
    except Exception as e:
        logger.warning(f"[{state['symbol']}] Arbitrage node failed: {e}")
...
def build_graph() -> StateGraph:
    ...
    # Fan-out: all five agents run in parallel from start
    ...
    workflow.add_edge("__start__", "arbitrage")

    # Fan-in: all five → debate
    ...
    workflow.add_edge("arbitrage", "debate")
...
async def run_analysis_cycle(
    ...
    # Determine execute_on from arbitrage signal if available
    arb = final_state.get("arbitrage")
    if decision.decision in ("SELL", "GTT_STOP"):
        execute_on = arb.recommended_sell_exchange if arb else "NSE"
    elif decision.decision in ("GTT_TARGET",):
        execute_on = arb.recommended_buy_exchange if arb else "NSE"
    else:
        execute_on = "NSE"
