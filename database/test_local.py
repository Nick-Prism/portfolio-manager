"""
database/test_local.py — Full end-to-end test of Person 3's code.
...
Run from project root: python -m database.test_local
"""
...
def test_price_data():
    ...
    from database.data.fetchers import get_price_data, get_current_price
    ...
    if df is not None and not df.empty:
        print(f"    ✅  {len(df)} rows fetched. Latest close: ₹{df[col].iloc[-1]:.2f}")
    else:
...
async def test_fundamentals():
    ...
    from database.data.fetchers import get_fundamentals
...
def test_indicators():
    ...
    from database.data.indicators import compute_all
...
async def test_mongo_write() -> str:
    ...
    from database.db.client import decisions_col
    from database.db.models import (DecisionDocument, TechnicalSignal, FundamentalSignal,
                            SentimentSignal, RiskSignal, NewsArticle, MACDData,
                            BollingerData, KeyLevels)

    if decisions_col is None:
        print("    ⚠️   MONGODB_URI not set — skipping write test")
        return ""
...
async def test_queries():
    print("\n🔍  Test 6: Dashboard queries")
    from database.db.queries import (get_last_decisions, get_decisions_by_symbol,
